...
function unlock_unified() {
    ...
    while true; do
        ...
        # --- 4.4. Bitwarden Authentication (The Source of Trust) ---
        ...
        if [[ $? -eq "$EXIT_SUCCESS" ]]; then
            # Save the session key to the RAM bridge for sharing across terminals.
    ...
    log_info "Synchronizing local database with Vaultwarden server..."
    if bw sync; then
        log_info "Vault successfully synchronized."
    else
        log_warn "Vault sync failed (Offline mode?). Continuing with last known local data."
    fi

    if [[ -z "$GPG_KEY" ]]; then
        log_err "GPG encryption key is missing. Cannot proceed with sync."
        exit "$EXIT_CRYPTO_ERR"
    fi
    
    # --- 4.7. Secret Retrieval ---
    log_info "Downloading environment data from vault (Item ID: $ITEM_ID)..."
    ITEM_JSON=$(bw get item "$ITEM_ID")
    if [[ $? -ne "$EXIT_SUCCESS" ]]; then 
        log_err "Retrieval failed: Item $ITEM_ID not found."
    ...
    # SECURITY: Use Atomic Swap to prevent readers (load.sh) from sourcing a partial file.
    ...
    # --- 4.10. Disk Persistence (Symmetric Encryption) ---
    log_info "Updating persistent encrypted disk cache backup..."
    if echo "$ENV_DATA" | gpg --quiet --batch --yes --passphrase "$GPG_KEY" --pinentry-mode loopback --symmetric --output "$CACHE_GPG"; then
        log_info "Local encrypted cache updated at $CACHE_GPG"
    else
        log_err "Disk backup failed: GPG encryption error."
        exit "$EXIT_CRYPTO_ERR"
    fi
